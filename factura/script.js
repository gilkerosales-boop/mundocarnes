...
function evaluarFormaPagoFactura(valor) {
  ...
  // Manejo de Cobro en Efectivo y Cálculo Automático de Vuelto
  if (valor === 'Efectivo Divisas' || valor === 'Efectivo Bolívares') {
    if (contEfectivo) {
      contEfectivo.classList.remove('hidden');
      configurarPanelCalculoEfectivo(valor);
    }
  } else if (valor !== 'Cashea' && valor !== 'Pago Mixto') {
    if (contEfectivo) contEfectivo.classList.add('hidden');
  }
}
...
function configurarPanelCalculoEfectivo(metodo) {
  ...
  const simbolo = document.getElementById('simboloMonedaRecibida');
  ...
  if (metodo === 'Efectivo Divisas') {
    if (badge) badge.textContent = "Divisas ($)";
    ...
    billetes.forEach(b => {
      if (b >= totalUSD || b === 5 || b === 10 || b === 20 || b === 50 || b === 100) {
        htmlBilletes += `<button type="button" class="btn-billete-rapido" onclick="seleccionarBilleteRapido(${b})">$${b}</button>`;
      }
    });
  } else {
    if (badge) badge.textContent = "Bolívares (Bs)";
    if (simbolo) simbolo.textContent = "Bs";
    if (inputMonto) inputMonto.value = totalBS.toFixed(2);
    if (selectVuelto) selectVuelto.value = "BS_EFECTIVO";

    htmlBilletes += `<button type="button" class="btn-billete-rapido active" onclick="seleccionarBilleteRapido(${totalBS})">Exacto (Bs. ${totalBS.toLocaleString('es-VE', { minimumFractionDigits: 2 })})</button>`;
    const multiplos = [100, 200, 500, 1000, 2000, 5000];
    multiplos.forEach(m => {
      if (m >= totalBS) {
        htmlBilletes += `<button type="button" class="btn-billete-rapido" onclick="seleccionarBilleteRapido(${m})">Bs. ${m}</button>`;
      }
    });
  }

  if (contBotones) contBotones.innerHTML = htmlBilletes;
  calcularVueltoEfectivo();
}

function seleccionarBilleteRapido(monto) {
  const inputMonto = document.getElementById('inputMontoRecibidoEfectivo');
  if (inputMonto) {
    inputMonto.value = parseFloat(monto).toFixed(2);
  }
  document.querySelectorAll('.btn-billete-rapido').forEach(b => b.classList.remove('active'));
  calcularVueltoEfectivo();
}
window.seleccionarBilleteRapido = seleccionarBilleteRapido;

// Calcular vuelto dinámico con soporte total para Efectivo Directo, Cashea y Pagos Mixtos
function calcularVueltoEfectivo() {
  ...
  // CASO 1: CASHEA O PAGO MIXTO CON EFECTIVO
  if (metodo === 'Cashea' || metodo === 'Pago Mixto') {
    let sumaNoEfectivoUSD = 0;
    let montoEfectivoIngresadoUSD = 0;

    const filas = document.querySelectorAll('.fila-pago-mixto');
    filas.forEach(f => {
      let met = f.querySelector('.select-metodo-mixto')?.value || "";
      let mto = parseFloat(f.querySelector('.input-monto-mixto')?.value) || 0;
      if (met.includes("Efectivo") || met === "Efectivo Divisas" || met === "Efectivo Bolívares") {
        montoEfectivoIngresadoUSD += (met === "Efectivo Bolívares" ? (tasa > 0 ? mto / tasa : 0) : mto);
      } else {
        sumaNoEfectivoUSD += (METODOS_BS.includes(met) ? (tasa > 0 ? mto / tasa : 0) : mto);
      }
    });

    const inicialRequeridaUSD = Math.max(0, totalUSD - sumaNoEfectivoUSD);
    vueltoTotalUSD = Math.max(0, montoEfectivoIngresadoUSD - inicialRequeridaUSD);
    vueltoTotalBS = vueltoTotalUSD * tasa;

  } else if (metodo === 'Efectivo Divisas') {
    // CASO 2: EFECTIVO DIVISAS PURO
    vueltoTotalUSD = Math.max(0, montoEntregado - totalUSD);
    vueltoTotalBS = vueltoTotalUSD * tasa;
  } else {
    // CASO 3: EFECTIVO BOLÍVARES PURO
    vueltoTotalBS = Math.max(0, montoEntregado - totalBS);
    vueltoTotalUSD = tasa > 0 ? (vueltoTotalBS / tasa) : 0;
  }

  if (medioVuelto === 'EXACTO' || (vueltoTotalUSD < 0.01 && vueltoTotalBS < 0.01)) {
    if (lblPrincipal) lblPrincipal.textContent = "Bs. 0,00";
    if (lblSecundario) lblSecundario.textContent = "($0.00)";
  ...
  // VUELTO MIXTO ($ EN DIVISAS + RESTANTE EN BS / PAGO MÓVIL)
  if (medioVuelto === 'MIXTO') {
    let parteDivisasUSD = parseFloat(inputParteUSD?.value);
    if (isNaN(parteDivisasUSD) || parteDivisasUSD < 0 || parteDivisasUSD > vueltoTotalUSD || inputParteUSD?.value === "") {
...
function calcularTotalPagoMixto() {
  ...
  if (hayLineaEfectivo && restanteUSD < -0.01) {
    ...
    if (contEfectivo) {
      ...
      const selectVuelto = document.getElementById('selectMedioVuelto');
      if (selectVuelto && selectVuelto.value === 'EXACTO') {
        selectVuelto.value = 'BS_EFECTIVO';
      }
      calcularVueltoEfectivo();
    }
  } else {
...
async function emitirFacturaFinal() {
  ...
  try {
    ...
    // Capturar datos de efectivo entregado y vuelto otorgado
    const datosVuelto = calcularVueltoEfectivo();
...
function renderizarTicketTermicoHTML(d) {
  ...
  if (d.modoFiscal) {
    ...
    // Renglones de vuelto exacto en ticket (Directo, Cashea y Mixto)
    let renglonesVueltoTicket = "";
    const vTotalUSD = parseFloat(d.vueltoTotalUSD || d.vueltoUSD || 0) || 0;
    const vTotalBS = parseFloat(d.vueltoTotalBS || d.vueltoBS || 0) || 0;
    const vUSD = parseFloat(d.vueltoUSD) || 0;
    const vBS = parseFloat(d.vueltoBS) || 0;
    const mEntregado = parseFloat(d.montoEntregado) || 0;

    if (vTotalUSD > 0.009 || vTotalBS > 0.01 || vUSD > 0.009 || vBS > 0.01) {
      const entregadoTxt = (formaUpper.includes("DIVISAS") || d.medioVuelto === "USD_EFECTIVO" || d.medioVuelto === "MIXTO" || mEntregado > 0)
        ? ("$" + (mEntregado > 0 ? mEntregado : (d.totalUSD + vTotalUSD)).toFixed(2))
        : ("Bs. " + (mEntregado > 0 ? mEntregado : (d.totalBs + vTotalBS)).toLocaleString('es-VE', { minimumFractionDigits: 2 }));
      ...
      if (d.medioVuelto === "MIXTO") {
        ...
        vueltoTxt = "$" + (vTotalUSD > 0 ? vTotalUSD : vUSD).toFixed(2);
      } else {
        vueltoTxt = "Bs. " + (vTotalBS > 0 ? vTotalBS : vBS).toLocaleString('es-VE', { minimumFractionDigits: 2 });
      }

      renglonesVueltoTicket = `
        <div class="pp9-fila-item small text-muted">
          <span>PAGÓ CON:</span>
          <span>${entregadoTxt}</span>
        </div>
        <div class="pp9-fila-item small text-primary pp9-bold">
          <span>VUELTO:</span>
          <span>${vueltoTxt}</span>
        </div>`;
    }
...
let piezasCanalPesadas = []; // Registro interactivo pieza a pieza en báscula de riel/gancho
...
function abrirModalEntradaMercancia() {
  ...
  const inpPesoFactura = document.getElementById('entradaPesoFacturaProveedorInput');
  ...
  // Campos de desposte y báscula
  const inpPieza = document.getElementById('inputPesoPiezaCanal');
  const inpCostoKiloCanal = document.getElementById('desposteCostoKiloCanalInput');
  const selSello = document.getElementById('desposteSelloSelect');
  const inpMerma = document.getElementById('despostePorcMermaEstimadaInput');

  if (inpProv) inpProv.value = "";
  if (inpGuia) inpGuia.value = "";
  if (inpPesoFactura) inpPesoFactura.value = "";
  ...
  if (inpPieza) inpPieza.value = "";
  if (inpCostoKiloCanal) inpCostoKiloCanal.value = "";
  if (selSello) selSello.value = "MORADO";
  if (inpMerma) inpMerma.value = "20.0";
...
function previsualizarFotoFacturaEntrada(fileInput) {
  const previewCont = document.getElementById('contenedorPreviewFotoFactura');
  const previewImg = document.getElementById('previewFotoFacturaImg');
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    reader.onload = function(e) {
      fotoFacturaBase64 = e.target.result;
      if (previewImg) previewImg.src = fotoFacturaBase64;
      if (previewCont) previewCont.classList.remove('hidden');
    };
    reader.readAsDataURL(file);
  }
}
window.previsualizarFotoFacturaEntrada = previsualizarFotoFacturaEntrada;

function limpiarFotoFacturaEntrada() {
  fotoFacturaBase64 = null;
  const inpFoto = document.getElementById('entradaFotoFacturaInput');
  const previewCont = document.getElementById('contenedorPreviewFotoFactura');
  const previewImg = document.getElementById('previewFotoFacturaImg');
...
window.limpiarFotoFacturaEntrada = limpiarFotoFacturaEntrada;

function ampliarFotoFacturaEntrada(imgSrc) {
  if (!imgSrc) return;
  const w = window.open("");
  w.document.write(`
    <html style="background:#0f172a; margin:0; height:100%; display:flex; justify-content:center; align-items:center;">
      <head><title>Factura / Guía Adjunta</title></head>
      <body style="margin:0; text-align:center;">
        <img src="${imgSrc}" style="max-width:95vw; max-height:95vh; object-fit:contain; border-radius:8px; box-shadow:0 10px 30px rgba(0,0,0,0.5);">
      </body>
    </html>
  `);
}
...
function filtrarMenuDesplegableProductos() {
  ...
  let filtrados = prodsDisponibles.filter(p => {
    ...
    const plu = String(p.codigoPLU || "").toLowerCase();
    return nom.includes(textoBuscar) || plu.includes(textoBuscar);
  });
...
function actualizarAyudaEntradaProducto() {
  ...
  if (inputCosto) {
    inputCosto.placeholder = (unidad === 'unidades') ? '0.00 / ud' : '0.00 / Kg';
  }
  if (ayudaStock) {
    ayudaStock.innerHTML = `Stock actual de <strong>${prodNom}</strong>: <span class="fw-bold text-success">${stockActual} ${unidadTxt}</span>.`;
  }
  if (ayudaPrecioVenta) {
    ayudaPrecioVenta.innerHTML = `Precio Venta Público: <span class="text-danger fw-bold">$${precioVenta.toFixed(2)}/${unidadTxt}</span>`;
  }
}
window.actualizarAyudaEntradaProducto = actualizarAyudaEntradaProducto;

function reconciliarTotalesFacturaEntrada() {
  const inpMontoTot = document.getElementById('entradaMontoTotalFacturaInput');
  const lblMontoFact = document.getElementById('lblResumenMontoFactura');
  const lblTotalLote = document.getElementById('lblResumenTotalLote');
  const lblDiferencia = document.getElementById('lblResumenDiferencia');

  const montoDeclarado = parseFloat(inpMontoTot ? inpMontoTot.value : 0) || 0;
  let sumaLote = 0;
  loteEntradaMercancia.forEach(it => {
    sumaLote += parseFloat(it.subtotal) || 0;
  });

  const diferencia = montoDeclarado - sumaLote;
  const difAbs = Math.abs(diferencia);

  if (lblMontoFact) lblMontoFact.textContent = `$${montoDeclarado.toFixed(2)}`;
  if (lblTotalLote) lblTotalLote.textContent = `$${sumaLote.toFixed(2)}`;

  if (lblDiferencia) {
    if (montoDeclarado === 0 && sumaLote === 0) {
      lblDiferencia.textContent = "$0.00";
      lblDiferencia.className = "fs-6 ms-1 num-legible text-dark";
    } else if (difAbs < 0.01) {
      lblDiferencia.textContent = "✔ Cuadrado ($0.00)";
      lblDiferencia.className = "fs-6 ms-1 num-legible text-success fw-bold";
    } else if (diferencia > 0) {
      lblDiferencia.textContent = `Falta cargar: $${difAbs.toFixed(2)}`;
      lblDiferencia.className = "fs-6 ms-1 num-legible text-warning fw-bold";
    } else {
      lblDiferencia.textContent = `Exceso en lote: -$${difAbs.toFixed(2)}`;
...
function agregarProductoALoteEntrada() {
  ...
  if (isNaN(cantRecibida) || cantRecibida <= 0) {
    if (errDiv) {
      errDiv.textContent = "Ingrese una cantidad recibida mayor a cero.";
      errDiv.classList.remove('hidden');
    }
    return;
  }
  ...
  if (isNaN(costoUnitario) || costoUnitario <= 0) {
    if (errDiv) {
      errDiv.textContent = "Ingrese el precio unitario del producto según factura mayor a cero.";
      errDiv.classList.remove('hidden');
    }
    return;
  }
  ...
  if (existenteIndex !== -1) {
    ...
    loteEntradaMercancia[existenteIndex].costoUnitario = costoUnitario;
    loteEntradaMercancia[existenteIndex].subtotal = loteEntradaMercancia[existenteIndex].cantRecibida * costoUnitario;
  ...
  loteEntradaMercancia.forEach((it, idx) => {
    ...
    html += `
      <tr>
        ...
        <td class="text-end fw-bold text-primary num-legible">$${it.costoUnitario.toFixed(2)}</td>
        <td class="text-end fw-bold text-dark num-legible">$${it.subtotal.toFixed(2)}</td>
...
// ==========================================================================
// MÓDULO EXCLUSIVO: DESPOSTE DE RES EN CANAL Y PESAJE PIEZA A PIEZA EN GANCHO
// ==========================================================================

// Matriz zootécnica de rendimiento estándar para res en canal (% sobre peso útil de carne)
const MATRIZ_RENDIMIENTO_RES = {
  "BISTEC": 0.150,
  "MOLIDA": 0.145,
  "GUISAR": 0.120,
  "MECHAR": 0.085,
  "SOLOMO DE CUERITO": 0.090,
  "PUNTA TRASERA": 0.035,
  "LOMITO": 0.020,
  "CHULETA DE RES": 0.050,
  "COSTILLA DE RES": 0.150,
  "LAGARTO CON HUESO": 0.070,
  "LAGARTO SIN HUESO": 0.045,
  "HUESO ROJO": 0.040
};

// 1. Agregar pieza individual pesada en gancho/báscula (opción [+])
function agregarPiezaPesadaCanal() {
  const inp = document.getElementById('inputPesoPiezaCanal');
  const peso = parseFloat(inp ? inp.value : 0);

  if (isNaN(peso) || peso <= 0) {
    mostrarAvisoFactura("Ingrese un peso válido para la pieza en gancho.");
    if (inp) inp.focus();
    return;
  }

  piezasCanalPesadas.push(peso);
  if (inp) {
    inp.value = "";
    inp.focus();
  }
...
window.agregarPiezaPesadaCanal = agregarPiezaPesadaCanal;

function eliminarPiezaPesadaCanal(idx) {
  if (idx >= 0 && idx < piezasCanalPesadas.length) {
    piezasCanalPesadas.splice(idx, 1);
    renderizarChipsPiezasCanal();
    reconciliarPesajesCanalFactura();
  }
}
window.eliminarPiezaPesadaCanal = eliminarPiezaPesadaCanal;

function limpiarTodasLasPiezasCanal() {
  if (piezasCanalPesadas.length === 0) return;
  if (confirm("¿Desea vaciar todos los pesajes de piezas en gancho registrados?")) {
    piezasCanalPesadas = [];
    renderizarChipsPiezasCanal();
    reconciliarPesajesCanalFactura();
  }
}
window.limpiarTodasLasPiezasCanal = limpiarTodasLasPiezasCanal;

function renderizarChipsPiezasCanal() {
  const cont = document.getElementById('contenedorChipsPiezasCanal');
  const badgeCnt = document.getElementById('cntPiezasCanalPesadas');
  if (!cont) return;

  if (badgeCnt) badgeCnt.textContent = piezasCanalPesadas.length;

  if (piezasCanalPesadas.length === 0) {
    cont.innerHTML = `<span class="text-muted small p-1">No hay piezas pesadas. Ingrese el peso de cada media canal y presione ➕.</span>`;
    return;
  }

  let html = "";
  piezasCanalPesadas.forEach((p, idx) => {
    html += `
      <span class="badge bg-primary d-inline-flex align-items-center gap-1 p-2 shadow-sm" style="font-size: 0.78rem;">
        <span>#${idx + 1}: <strong>${p.toFixed(2)} Kg</strong></span>
        <button type="button" class="btn-close btn-close-white" style="font-size: 0.55rem;" onclick="eliminarPiezaPesadaCanal(${idx})" title="Quitar pieza"></button>
      </span>
    `;
  });
...
// 2. Conciliación entre Peso Facturado vs Total Pesado en Báscula de Gancho
function reconciliarPesajesCanalFactura() {
  const inpPesoFact = document.getElementById('entradaPesoFacturaProveedorInput');
  const lblFact = document.getElementById('lblPesajeFacturaProveedor');
  const lblBascula = document.getElementById('lblPesajeTotalBascula');
  const lblDif = document.getElementById('lblDiferenciaPesajeCanal');

  const pesoFactura = parseFloat(inpPesoFact ? inpPesoFact.value : 0) || 0;
  const totalBascula = piezasCanalPesadas.reduce((a, b) => a + b, 0);

  if (lblFact) lblFact.textContent = `${pesoFactura.toFixed(2)} Kg`;
  if (lblBascula) lblBascula.textContent = `${totalBascula.toFixed(2)} Kg`;

  const dif = totalBascula - pesoFactura;
  const difAbs = Math.abs(dif);

  if (lblDif) {
    if (pesoFactura === 0 && totalBascula === 0) {
      lblDif.textContent = "0.00 Kg";
      lblDif.className = "fs-6 ms-1 num-legible text-dark";
    } else if (difAbs < 0.05) {
      lblDif.textContent = "✔ Báscula Cuadrada con Factura (0.00 Kg)";
      lblDif.className = "fs-6 ms-1 num-legible text-success fw-bold";
    } else if (dif < 0) {
      lblDif.textContent = `Faltan pesarse: ${difAbs.toFixed(2)} Kg`;
      lblDif.className = "fs-6 ms-1 num-legible text-warning fw-bold";
    } else {
      lblDif.textContent = `Exceso en báscula: +${dif.toFixed(2)} Kg`;
      lblDif.className = "fs-6 ms-1 num-legible text-primary fw-bold";
    }
  }

  recalcularDesposteVirtualZootecnico();
}
window.reconciliarPesajesCanalFactura = reconciliarPesajesCanalFactura;

function actualizarMermaPorSello() {
  const sello = document.getElementById('desposteSelloSelect')?.value || "MORADO";
  const inputMerma = document.getElementById('despostePorcMermaEstimadaInput');

  if (inputMerma) {
    if (sello === "MORADO") inputMerma.value = "20.0";
    else if (sello === "ROJO") inputMerma.value = "23.0";
    else if (sello === "AZUL") inputMerma.value = "26.0";
  }

  recalcularDesposteVirtualZootecnico();
}
window.actualizarMermaPorSello = actualizarMermaPorSello;
...
function cargarTablaCortesDesposte() {
  ...
  // FILTRADO ESTRICTO: Excluir tajantemente cortes de cerdo/cochino, pollo y ahumados
  const terminosExcluidos = ["COCHINO", "CERDO", "POLLO", "AHUMADA", "CHULETA AHUMADA"];
  cortesCarnes = cortesCarnes.filter(c => {
    const n = c.nombre.toUpperCase();
    return !terminosExcluidos.some(t => n.includes(t));
  });
  ...
  cortesCarnes.forEach((corte, idx) => {
    ...
    const porcZootecnico = MATRIZ_RENDIMIENTO_RES[corte.nombre] ? (MATRIZ_RENDIMIENTO_RES[corte.nombre] * 100).toFixed(1) : "0.0";
    ...
    html += `
      <tr class="fila-corte-desposte" data-nombre="${safeNom}" data-unidad="${corte.unidad}" data-stock="${corte.stock}">
        ...
        <td class="text-center fw-bold text-primary num-legible" id="desposte-porc-${idx}">${porcZootecnico}%</td>
        <td class="text-center">
          <input type="number" step="0.01" min="0" class="form-control form-control-sm text-center fw-bold input-kilos-desposte num-legible" 
                 id="desposte-kilos-${idx}" placeholder="0.00" oninput="calcularBalanceRendimientoDesposte(true)">
        </td>
...
function recalcularDesposteVirtualZootecnico() {
  // El peso base del desposte se toma preferiblemente del pesaje real de piezas en báscula
  let pesoCanal = piezasCanalPesadas.reduce((a, b) => a + b, 0);
  if (pesoCanal === 0) {
    pesoCanal = parseFloat(document.getElementById('entradaPesoFacturaProveedorInput')?.value) || 0;
  }

  const porcMerma = parseFloat(document.getElementById('despostePorcMermaEstimadaInput')?.value) || 20.0;
  const mermaEstimadaKg = pesoCanal * (porcMerma / 100);
  const kilosUtilesCarne = Math.max(0, pesoCanal - mermaEstimadaKg);
  const porcUtil = Math.max(0, 100 - porcMerma);

  // Actualizar indicadores superiores
  const lblMermaEst = document.getElementById('lblDesposteMermaEstimadaKg');
  const lblUtiles = document.getElementById('lblDesposteKilosUtiles');
  if (lblMermaEst) lblMermaEst.textContent = `${mermaEstimadaKg.toFixed(2)} Kg`;
  if (lblUtiles) lblUtiles.textContent = `${kilosUtilesCarne.toFixed(2)} Kg (${porcUtil.toFixed(1)}%)`;

  // Distribuir proporcionalmente los kilos útiles entre los cortes vacunos
  ...
  filas.forEach((f, idx) => {
    ...
    const factorCorte = MATRIZ_RENDIMIENTO_RES[prodNom] || 0;

    if (inputKg) {
      if (kilosUtilesCarne > 0 && factorCorte > 0) {
        const kilosProyectados = kilosUtilesCarne * factorCorte;
        inputKg.value = kilosProyectados.toFixed(2);
      } else if (kilosUtilesCarne <= 0) {
        inputKg.value = "";
      }
    }
  });

  calcularBalanceRendimientoDesposte(false);
}
window.recalcularDesposteVirtualZootecnico = recalcularDesposteVirtualZootecnico;
...
function calcularBalanceRendimientoDesposte(esEdicionManual = false) {
  let pesoCanal = piezasCanalPesadas.reduce((a, b) => a + b, 0);
  if (pesoCanal === 0) {
    pesoCanal = parseFloat(document.getElementById('entradaPesoFacturaProveedorInput')?.value) || 0;
  }
...
function limpiarPesajesDesposte() {
  ...
  calcularBalanceRendimientoDesposte(true);
  mostrarAvisoFactura("🔄 Pesajes de desposte reiniciados.");
}
window.limpiarPesajesDesposte = limpiarPesajesDesposte;

// ==========================================================================
// LOTES DE DESPOSTE EN STANDBY Y LIQUIDACIÓN CON MERMA REAL
// ==========================================================================
async function actualizarContadorLotesStandby() {
  try {
    const lotes = await dbGetAll("lotes_desposte");
    const pendientes = (lotes || []).filter(l => l.estatus === "STANDBY").length;
    const badge = document.getElementById('cntLotesDesposteStandby');
    if (badge) badge.textContent = pendientes;
  } catch (e) {}
}

async function abrirModalLotesDesposteStandby() {
  const tbody = document.getElementById('tablaLotesDesposteStandby');
  const panelLiq = document.getElementById('panelLiquidacionLoteSeleccionado');
  const errDiv = document.getElementById('errorModalStandbyDesposte');
  if (errDiv) errDiv.classList.add('hidden');
  if (panelLiq) panelLiq.classList.add('hidden');

  try {
    const lotes = await dbGetAll("lotes_desposte");
    const lotesStandby = (lotes || []).filter(l => l.estatus === "STANDBY");

    if (lotesStandby.length === 0) {
      tbody.innerHTML = `<tr><td colspan="9" class="text-center text-muted py-4">No hay lotes de desposte pendientes en espera (Standby).</td></tr>`;
    } else {
      let html = "";
      lotesStandby.forEach(l => {
        html += `
          <tr>
            <td class="fw-bold text-primary num-legible">${l.nroGuia}</td>
            <td class="small num-legible">${l.fecha}</td>
            <td class="fw-bold text-dark text-truncate" style="max-width:160px;" title="${l.proveedor}">${l.proveedor}</td>
            <td class="text-center"><span class="badge bg-dark">${l.sello || 'ROJO'}</span></td>
            <td class="text-center fw-bold num-legible">${parseFloat(l.pesoCanal || 0).toFixed(2)} Kg</td>
            <td class="text-center fw-bold text-success num-legible">${parseFloat(l.kilosUtilesEstimados || 0).toFixed(2)} Kg</td>
    ...
    actualizarContadorLotesStandby();
    bootstrap.Modal.getOrCreateInstance(document.getElementById('modalLotesDesposteStandby')).show();
  } catch (err) {
    console.error("Error al abrir lotes en standby:", err);
  }
}
...
async function seleccionarLoteParaLiquidar(idLote) {
  try {
    ...
    document.getElementById('desposteLoteIdLiquidando').value = lote.id;
    document.getElementById('lblLoteLiquidandoGuia').textContent = `${lote.nroGuia} (${lote.proveedor})`;
    ...
    document.getElementById('panelLiquidacionLoteSeleccionado').classList.remove('hidden');
  } catch (e) {
    console.error(e);
  }
}
window.seleccionarLoteParaLiquidar = seleccionarLoteParaLiquidar;

function cancelarSeleccionLoteLiquidacion() {
  document.getElementById('panelLiquidacionLoteSeleccionado').classList.add('hidden');
  document.getElementById('desposteLoteIdLiquidando').value = "";
}
window.cancelarSeleccionLoteLiquidacion = cancelarSeleccionLoteLiquidacion;

async function calcularDesviacionMermaLiquidacion() {
...
window.calcularDesviacionMermaLiquidacion = calcularDesviacionMermaLiquidacion;
...
async function confirmarLiquidacionRealLote() {
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = "🔒 Liquidar y Ajustar Inventario"; }
    console.error("Error al liquidar lote:", errLiq);
    if (errorDiv) {
      errorDiv.textContent = "Error al liquidar lote: " + errLiq.message;
      errorDiv.classList.remove('hidden');
    }
  }
}
...
async function ejecutarIngresoRecepcionFinal() {
  ...
  // --- MODO DESPOSTE VIRTUAL EN STANDBY ---
  ...
  const pesoFacturaProveedor = parseFloat(document.getElementById('entradaPesoFacturaProveedorInput')?.value) || 0;
  ...
  if (!nroGuia) {
    if (errorDiv) {
      errorDiv.textContent = "El campo 'N.° Guía / Factura' es obligatorio.";
      errorDiv.classList.remove('hidden');
    }
  ...
  if (pesoFacturaProveedor <= 0) {
    if (errorDiv) {
      errorDiv.textContent = "El campo 'Peso Factura (Kg)' es obligatorio y debe ser mayor a 0.";
      errorDiv.classList.remove('hidden');
    }
    document.getElementById('entradaPesoFacturaProveedorInput')?.focus();
    return;
  }

  if (montoTotalFactura <= 0) {
    if (errorDiv) {
      errorDiv.textContent = "El campo 'Costo Total ($)' es obligatorio y debe ser mayor a 0.";
      errorDiv.classList.remove('hidden');
    }
  ...
  // Determinar peso total pesado en báscula de gancho
  let pesoCanal = piezasCanalPesadas.reduce((a, b) => a + b, 0);
  if (pesoCanal === 0) {
    pesoCanal = pesoFacturaProveedor;
  }

  // Conciliación de peso pesado vs peso facturado
  const difPeso = Math.abs(pesoCanal - pesoFacturaProveedor);
  if (difPeso > 0.5) {
    const continuarDiferencia = confirm(`⚠️ AVISO DE DIFERENCIA EN BÁSCULA:\nEl total pesado en gancho (${pesoCanal.toFixed(2)} Kg) difiere del peso facturado en guía (${pesoFacturaProveedor.toFixed(2)} Kg) por una diferencia de ${difPeso.toFixed(2)} Kg.\n\n¿Desea confirmar e ingresar el desposte con el peso pesado en báscula?`);
    if (!continuarDiferencia) return;
  }

  // Recolectar cortes proyectados de res
  const filas = document.querySelectorAll('.fila-corte-desposte');
  ...
  if (cortesAcreditables.length === 0 || totalKilosDespostados <= 0) {
    if (errorDiv) {
      errorDiv.textContent = "Verifique la proyección zootécnica de cortes vacunos.";
      errorDiv.classList.remove('hidden');
    }
    return;
  }
  ...
  if (btn) {
    btn.disabled = true;
    btn.textContent = "Acreditando existencias a vitrina...";
  }
  ...
  try {
    ...
    const nuevoLoteDesposte = {
      ...
      proveedor: proveedor,
      nroGuia: nroGuia,
      pesoFacturaProveedor: pesoFacturaProveedor,
      pesoCanal: pesoCanal,
      ...
      cortesProyectados: cortesAcreditables,
      mermaRealKg: null,
      estatus: "STANDBY",
  ...
  if (navigator.onLine && supabaseClient) {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = "🥩 Cargar Desposte Virtual (Standby)";
    }
    ...
    console.error("Error al procesar desposte virtual:", err);
    if (errorDiv) {
      errorDiv.textContent = "Error al acreditar desposte virtual: " + err.message;
      errorDiv.classList.remove('hidden');
    }
  }
}
